...
if __name__ == '__main__':
    ...
    model_path = './models'
    # model_name = 'cont_{0}_fold_{1}.tar'.format(arch,str(id_k_fold))
    model_names = ['cont_upper_fold_6_best.tar','cont_lower_fold_6_best.tar']
    archs =['Upper', 'Lower']
    mesh_path = './ground-truth'  # need to modify
    sample_filenames = [] # need to modify
    for idx_arch in range(len(archs)):
        arch=archs[idx_arch]

        for i in glob.glob('./ground-truth/*.vtp'):
            a = i.split('\\')[-1]
            if arch.lower() in a.lower():
                sample_filenames.append(a)
        ...
        num_classes = 15
        num_channels = 15
        model_name = 'cont_{0}_fold_6_best.tar'.format(arch.lower())
        # model_name=model_names[idx_arch]
        # set model
        ...
        with torch.no_grad():
            for i_sample in sample_filenames:
                if arch.lower() in i_sample.lower():
                    ...
                    mesh3.celldata.remove('Normal')
                    vedo.write(mesh3, os.path.join(output_path, '{}.vtp'.format(i_sample[:-4])))
