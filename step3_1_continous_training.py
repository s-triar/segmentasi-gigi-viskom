...
if __name__ == '__main__':
    ...
    for a in archs:
        for fld in folds:
            ...
            arch=a
            id_k_fold=fld
            id_k_fold_before=fld-1
            train_list = './list_input/{0}_train_list_{1}.csv'.format(arch, str(id_k_fold))
            val_list = './list_input/{0}_val_list_{1}.csv'.format(arch, str(id_k_fold))
            
            # JIKA TERDAPAT ERROR 1 YANG SAYA TEMUI ADALAH HAPUS 1 BARIS DI FILE CSV train_list 
            # YANG JUMLAH BARISNYA 33 SEHINGGA MENJADI 32 
            
            previous_check_point_path = './models'
            previous_check_point_name = 'cont_{0}_fold_{1}.tar'.format(arch,str(id_k_fold_before))
            if id_k_fold_before==0:
                previous_check_point_name = 'cont_{0}_fold.tar'.format(arch)
            ...
            # re-load
            checkpoint = torch.load(os.path.join(previous_check_point_path, previous_check_point_name), map_location='cuda')
            ...
            print('Training model...', arch)
            print("epoch init", epoch_init)
            class_weights = torch.ones(num_classes).to(device, dtype=torch.float)
            for epoch in range(epoch_init, epoch_init+num_epochs):
                ...
                for i_batch, batched_sample in enumerate(train_loader):
                    # print(batched_sample['labels'].shape)
                ...
                with torch.no_grad():
                    ...
                    for i_batch, batched_val_sample in enumerate(val_loader):
                        ...
                        if i_batch % num_batches_to_print == num_batches_to_print-1:  # print every N mini-batches
                            print('[Epoch: {0}/{1}, Val batch: {2}/{3}] val_loss: {4}, val_dsc: {5}, val_sen: {6}, val_ppv: {7}'.format(epoch+1, epoch_init+num_epochs, i_batch+1, len(val_loader), running_val_loss/num_batches_to_print, running_val_mdsc/num_batches_to_print, running_val_msen/num_batches_to_print, running_val_mppv/num_batches_to_print))
                            running_val_loss = 0.0
                            running_val_mdsc = 0.0
                            running_val_msen = 0.0
                            running_val_mppv = 0.0
                    ...
                    print('*****\nEpoch: {}/{}, loss: {}, dsc: {}, sen: {}, ppv: {}\n         val_loss: {}, val_dsc: {}, val_sen: {}, val_ppv: {}\n*****'.format(epoch+1, epoch_init+num_epochs, losses[-1], mdsc[-1], msen[-1], mppv[-1], val_losses[-1], val_mdsc[-1], val_msen[-1], val_mppv[-1]))
                    if use_visdom:
                ...
                torch.save({'epoch': epoch+1,
                            ...
                            'val_msen': val_msen,
                            'val_mppv': val_mppv},
                            os.path.join(model_path, checkpoint_name+'.tar'))
                ...
                file_loss_metric = 'losses_metrics_vs_epoch_{0}_fold_{1}.csv'.format(arch, str(id_k_fold))
                stat.to_csv(file_loss_metric)
